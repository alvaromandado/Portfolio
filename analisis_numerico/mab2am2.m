...
% x'(t)=f(t,x(t)) en [t0,T]
% x(t0)=x0,
% con x0 en R^n, mediante un método PECE con predictor Adams-Bashford de 2 pasos y corrector Adams-Moulton de 2 pasos, inicializado con Euler mejorado.
%
% ENTRADA:
...
for i=1:N-1
    ...
    x(i+2,:)=x(i+1,:)+(h/2)*(3*y(2,:)-y(1,:)); y(3,:)=f(t(i+2),x(i+2,:));
    %Correccion
